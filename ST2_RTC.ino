...
void checktime()
{
	...
	// False on RTC when 24 mode selected
	TH_Not24_flag = bitRead(i2cData, 6);
...
void checkDate()
{
	...
	if (temp) {
		// Convert BCD month into integer month
		MonthCode = MonthCode + 10;
	}
...
void settimeNEW(uint8_t setselect)	// both min digits or both hour digits (advance one at a time)
{
	...
	case 2:
		...
		if (TH_Not24_flag) {	// 12 hours mode increment
...
void TwelveTwentyFourConvert()
{
	...
	if (TH_Not24_flag != NewTimeFormat) {
		if (NewTimeFormat)	// NewTimeFormat is same formate as TH_Not24_flag where H is 12 and LOW is 24
		{
			// ---------------- 24 -> 12
		...
		{
			// AM only check for 00 hours
			if ((PM_NotAM_flag == false) && (temphours == 12)) {
				temphours = 0;
			}
